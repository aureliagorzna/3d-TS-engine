...
const controls = (): void => {
  ...
  if (movementProps.rotateRight)
    cubes.forEach((cube: Cube) => cube.rotateFromCenter(1));
  if (movementProps.rotateLeft)
    cubes.forEach((cube: Cube) => cube.rotateFromCenter(-1));
  if (movementProps.lookRight)
    cubes.forEach((cube: Cube) => cube.rotateFromPlayer(-1));
...
class Cube {
  ...
  constructor(pos: position3d, color?: string, rotation?: number) {
    ...
    this.displayPos = pos;
    this.pos = {
      x: this.displayPos.x * 2,
      y: this.displayPos.y * 2,
      z: this.displayPos.z * 2,
    };

    this.setCorners();
    this.setCenter();

    this.radiusCube = getDistance(
    ...
    const radiusCenter = convert3dTo2d({
      x: this.center.x,
      y: this.center.y,
      z: 1,
    });
    const corner1Real = convert3dTo2d({
      x: this.corners[0].x,
      y: this.corners[0].y,
      z: 1,
    });
    this.radiusReal = getDistance(
      { x: this.center.x, y: this.center.y },
      { x: this.corners[0].x, y: this.corners[0].y }
    );
  ...
  public setCorners() {
    ...
    const corner2: position3d = {
      x: this.pos.x + CUBE_SIZE,
      y: this.pos.y,
      z: this.pos.z,
    };
    const corner3: position3d = {
      x: this.pos.x,
      y: this.pos.y + CUBE_SIZE,
      z: this.pos.z,
    };
    const corner4: position3d = {
      x: this.pos.x + CUBE_SIZE,
      y: this.pos.y + CUBE_SIZE,
      z: this.pos.z,
    };

    const corner5: position3d = {
      x: this.pos.x,
      y: this.pos.y,
      z: this.pos.z + CUBE_SIZE / 5,
    };
    const corner6: position3d = {
      x: this.pos.x + CUBE_SIZE,
      y: this.pos.y,
      z: this.pos.z + CUBE_SIZE / 5,
    };
    const corner7: position3d = {
      x: this.pos.x,
      y: this.pos.y + CUBE_SIZE,
      z: this.pos.z + CUBE_SIZE / 5,
    };
    const corner8: position3d = {
      x: this.pos.x + CUBE_SIZE,
      y: this.pos.y + CUBE_SIZE,
      z: this.pos.z + CUBE_SIZE / 5,
    };
  ...
  public fillColor() {
    ...
    const cornerA = {
      x: corner1.x,
      y: corner1.y + CUBE_SIZE / 2,
      z: corner1.z,
    };
    const cornerB = {
      x: corner2.x,
      y: corner2.y - CUBE_SIZE / 2,
      z: corner2.z,
    };
  ...
  public setCenter() {
    this.center = {
      x: this.pos.x + CUBE_SIZE / 2,
      y: this.pos.y + CUBE_SIZE / 2,
      z: this.pos.z + CUBE_SIZE / 10,
    };
  }
  ...
  public drawCenter(): void {
    ...
    const pos: position = getCirclePoint(
      this.angle,
  ...
  public rotateFromCenter(newAngle: number): void {
    const copyAngles: number[] = this.angles.map(
      (angle: number) => angle + newAngle
    );
...
const drawCircle = (pos: position, color: string) => {
  ...
  ctx.beginPath();
  ctx.arc(
    canvas.width / 2 + pos.x + cameraPos.x,
    canvas.height / 2 + pos.y + cameraPos.y,
    4,
    0,
    Math.PI * 2
  );
...
const addClassicTreeLeaves = (pos: position3d) => {
...
const laodObjects = () => {
  ...
  for (let i = 0; i < 360; i += 10) {
    ...
    const angle = getAngle({ x: pos.x, y: pos.y * 7 + 15 }, radius, {
      x: 0,
      y: 15,
    });
